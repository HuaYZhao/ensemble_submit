...
class FeatureSpec(object):
    ...
    def __init__(self, name, shape, default_value_fn=None, is_int_feature=True, is_var_len=False):
        self.name = name
        self.shape = shape
        self.default_value_fn = default_value_fn
        self.is_int_feature = is_int_feature
        self.is_var_len = is_var_len
    ...
    def get_default_values(self):
        ...
        if self.default_value_fn:
            return self.default_value_fn(shape)
        else:
            return np.zeros(
                shape, np.int64 if self.is_int_feature else np.float32)
