...
def build_pv_data(input_file):
    ...
    xargs = f"cp {output_pv_data_file} electra_data/finetuning_data/squad/dev.json"
    os.system(xargs)
...
def stage2_answer_verifier_step_one(input_file):
    ...
    for qid in stage1_qa_bagging_preds:
        ...
        if stage1_qa_bagging_odds[qid] > -1.7580918570120747:
            bagging_preds[qid] = ""
        bagging_odds[qid] = np.mean([odds[qid] for odds in all_odds])
...
def stage2_answer_verifier_step_two(input_file, output_file):
    ...
    for qid in stage2_step_one_bagging_preds:
        ...
        if stage2_step_one_bagging_odds[qid] > 1.071401908993721:
            bagging_preds[qid] = ""
        bagging_odds[qid] = np.mean([odds[qid] for odds in all_odds])
        if bagging_odds[qid] > 7.402858257293701:
            bagging_preds[qid] = ""
